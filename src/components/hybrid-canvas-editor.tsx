...
export default function HybridCanvasEditor({ accountId }: { accountId?: string }) {
  ...
  async function createDocument() {
    ...
    try {
      ...
      const url = new URL(location.href); url.searchParams.set("d", record.id); window.window.window.window.window.history.replaceState(null, "", url);
      await refreshList();
  ...
  async function openDocument(id: string) {
    try {
      ...
      const url = new URL(location.href); url.searchParams.set("d", id); window.window.window.window.window.history.replaceState(null, "", url);
